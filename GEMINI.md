...
## Checkpoint: July 11, 2026 (Dropsets, Advanced Metrics & Interactive History)
### 1. Schema & Logging Enhancements
- **SubSetLog & Rest Pauses:** Upgraded `SetLog` with an embedded `SubSetLog` list to seamlessly support dropsets and supersets. Added an optional `restPauseSeconds` (max 60s) field for precise inter-set rest tracking.
- **Smart Rep Defaulting:** Reps now dynamically default to the exact reps achieved on the final set of the previous workout, matching the existing weight-default behavior.
- **Fatigue Integration:** Refactored `_loadRecentMuscles` to loop through both standard `sets` and `subSets`, ensuring dropsets count accurately toward volume load, fatigue heatmaps, and >20-rep endurance multipliers.

### 2. UI, Menus & Interaction
- **Dynamic Create Button:** The long-press "Create & Add" folder assignment button is now disabled (greyed out) if the input text field is empty.
- **Detailed View Shortcut:** Injected a prominent "Detailed View" jump button at the top of the long-press context menu to rapidly route users to the exercise history screen.

### 3. Exercise History & Analytics
- **Dedicated History Screen:** Created `ExerciseHistoryScreen`, providing a chronologically sorted history of all logged sets and rest-pauses for any selected exercise.
- **Interactive `fl_chart` Integration:** Visualizes Max Weight and Total Reps using large dotted nodes (`FlDotCirclePainter`) instead of basic continuous lines.
- **Dynamic Axes & Tooltips:** Tapping legend items isolates specific lines and dynamically un-hides their respective Y-axis (lbs or reps). Custom tooltips permanently hover over the most recent data point for instant context, and are fully touch-interactive along the timeline.
- **Advanced Metric Cards:** Built a 4-card grid below the chart that calculates and displays: Total Workouts, All-Time Max Weight, Max Reps/Session, and Est. 1RM (Epley formula).

### 4. Aesthetics & Theming
- **5 New Premium Themes:** Expanded the custom Monkeytype-style UI theme catalog with Dracula, Gruvbox, Catppuccin, Tokyo Night, and Monokai.
...
- **In-Depth Seed Exercises & Auto-Sync:** Fully populated the master dictionary with comprehensive, anatomically detailed primary/secondary muscle designations utilizing sub-muscles. Built an automatic synchronization system on startup that updates existing exercise definitions with the latest anatomical changes without overwriting user folder structures or favorite stars.
...
- **Premium Long-Name visual formatting:** Integrated a visual layout parser (`FormattedExerciseName`) that breaks up long names into hierarchical structures: manufacturer tags (e.g. "Life Fitness"), the core exercise name, and parenthesis details as small clean subtitles.
- **Advanced Library Filtering & Sorting:** Overhauled the library filter modal. Added sorting choices (A-Z, Z-A, and Most Recent based on historical daily log dates) and manufacturer category checkboxes. Highlighted the filter icon dynamically when filters or sorting are active.
- **Monkeytype Custom Themes:** Implemented 11 custom visual themes (Carbon, Serika Dark, Nord, Cyberpunk, Laser, Sakura, Botanical, Modern Ink, Terra, Matrix, Red Dragon) available below the system themes in settings. Selectable via a premium horizontal scrolling preview picker containing actual color palettes and micro-animations. Saved locally to a flat settings text file without modifying Isar schema to prevent database migrations and keep user metrics safe.
